...
def show_window():
    ...
    if len(list_files) != 0:
        ...
        for j in range(0, len(list_files)):
            if j == 0: temp_text = "'" + list_files[j].name + "' — " + address(list_files[j].begin) + "_" + address(list_files[j].end)  + ", размер файла : " + str(list_files[j].end - list_files[j].begin) + " Кбайт";
            else:
                temp_text = "'" + list_files[j].name + "' — " + address(list_files[j].begin) + "_" + address(list_files[j].end)  + ", размер файла : " + str(list_files[j].end - list_files[j].begin + 1) + " Кбайт";
...
def info_window(temp_text):
    ...
    info_window.wm_geometry("+%d+%d" % (info_window.winfo_screenwidth() / 2 - 1.1 * info_window.winfo_reqwidth(),info_window.winfo_screenheight() / 2 - 0.5 * info_window.winfo_reqheight()))
...
ax.spines['bottom'].set_position(('data', 1000))
ax.spines['left'].set_position(('data', 1000))
text1X = - 0.7; text1Y = 1.4
ax.text(text1X, text1Y, "— Занято : 0 Кбайт")
plt.plot(text1X - 0.1, text1Y + 0.05, "o", c=color_insert, ms=8);
text2X = text1X; text2Y = text1Y - 0.2
...
window.focus_force()
window.wm_geometry("+%d+%d" % (window.winfo_screenwidth() / 2  - 2*window.winfo_reqwidth(), window.winfo_screenheight()  / 2  - window.winfo_reqheight()))
window.mainloop();
